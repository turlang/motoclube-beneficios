...
async function ensureDemoPartner() {
  ...
  if (!partner) {
    partner = new Partner({
      ...
      categoria: "oficina",
      telefone: "",
      endereco: { cidade: "São Paulo", bairro: "Centro", logradouro: "" },
      ativo: true
    });
    await partner.save();
  }
...
async function ensureClubContent() {
  ...
  for (const officer of officers) {
    await ClubOfficer.findOneAndUpdate(
      { cargo: officer.cargo },
      { $setOnInsert: { ...officer, ativo: true } },
      { upsert: true, new: true, runValidators: true }
    );
  }
  ...
  for (const event of events) {
    await ClubEvent.findOneAndUpdate(
      { titulo: event.titulo },
      { $setOnInsert: { ...event, ativo: true } },
      { upsert: true, new: true, runValidators: true }
    );
  }
  ...
  for (const post of posts) {
    await ClubPost.findOneAndUpdate(
      { titulo: post.titulo },
      { $setOnInsert: { ...post, ativo: true } },
      { upsert: true, new: true, runValidators: true }
    );
  }
}
...
async function main() {
  ...
  console.log("- Conteúdo institucional: perfil, comando, eventos e notícias");
}
