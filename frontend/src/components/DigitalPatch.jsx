...
export function DigitalPatch({ user, active = true, children }) {
  ...
  return (
    <section className={["digital-patch", active ? "digital-patch-active" : "digital-patch-inactive"].join(" ")}>
      ...
      <div className="patch-rocker patch-rocker-top">
        <span>IRMÃOS DO ASFALTO</span>
      </div>

      <div className="relative z-10 px-4 pb-4 pt-14">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-500">Apelido de estrada</p>
            <h2 className={[
              "mt-1 text-3xl font-black uppercase leading-none",
              active ? "text-white" : "text-zinc-500"
            ].join(" ")}>{user.apelidoEstrada}</h2>
            <div className="mt-2 flex gap-1">
              {Array.from({ length: 4 }).map((_, index) => (
                <Star
                  ...
                  className={[
                    "h-3.5 w-3.5",
                    index < stars && active ? "fill-amber-400 text-amber-400" : "text-zinc-700"
                  ].join(" ")}
                />
              ))}
            </div>
          ...
          <div className={[
            ...
            active ? "border-amber-400/25 bg-amber-400/10" : "border-zinc-800 bg-zinc-900/70"
          ].join(" ")}>
        ...
        <div className="relative mt-3 flex justify-center py-1">
          ...
          {user.patente === "Diretoria" && (
            <div className="absolute right-4 top-6 grid h-10 w-10 place-items-center rounded-full border border-amber-400/30 bg-black/80 text-amber-300">
        ...
        <div className={[
          "mt-3 flex items-center justify-center gap-2 rounded-2xl border px-3 py-2.5",
          active
          ...
          <span className="text-[10px] font-black uppercase tracking-[0.22em]">
            {active ? "Escudo ativo • benefício liberado" : "Escudo bloqueado • assinatura inativa"}
          </span>
        </div>

        {children}

        <div className="patch-motto mt-4">
          <span>HONRA</span><i /> <span>LEALDADE</span><i /> <span>IRMANDADE</span>
        </div>
      </div>

      <div className="patch-rocker patch-rocker-bottom">
        <span>MOTOCLUBE • BRASIL</span>
      </div>
    </section>
  );
}
  ...
  return (
    <div className="rounded-2xl border border-white/5 bg-black/35 p-3">
      <div className="flex items-center gap-2 text-amber-300/80">
        {icon}
        <span className="text-[9px] font-black uppercase tracking-[0.24em] text-zinc-600">{label}</span>
      </div>
